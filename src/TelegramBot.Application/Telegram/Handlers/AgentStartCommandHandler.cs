...
public class AgentStartCommandHandler(IDbUnitOfWork dbUnitOfWork, ITelegramService telegramService, ILocalizationService localization) : ITelegramUpdateHandler
{
    public bool CanHandle(TelegramUser? user, TelegramUpdateContext ctx)
    {
        return user != null && user.Role == UserRole.Agent && ctx.MessageText == BotCommands.Start
